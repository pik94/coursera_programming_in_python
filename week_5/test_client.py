...
"""
import sys
sys.path.append("/home/pik/data/workspace/coursera/python/")
from week_5.client_coursera import Client, ClientError, ClientProtocolError
...
def run(host, port):
    ...
    #     print("Ошибка общения с сервером:", err.__class__,":",err)
    #     # sys.exit(1)
    ...
    try:
        ...
        if metrics != expected_metrics:
            print("client.get('*') вернул неверный результат. Ожидается:", expected_metrics, ". Получено:", metrics)
            sys.exit(1)
    except Exception as err:
    ...
    try:
        ...
        if metrics != expected_metrics:
            print("client.get('k2') вернул неверный результат. Ожидается:", expected_metrics, ". Получено:", metrics)
            sys.exit(1)
    except Exception as err:
    ...
    try:
        ...
        if result != {}:
            print("Ошибка вызова метода get с ключом, который еще не был добавлен. Ожидается: пустой словарь. Получено:", result)
            sys.exit(1)
    except Exception as err:
